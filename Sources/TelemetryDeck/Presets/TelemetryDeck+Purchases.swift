...
extension TelemetryDeck {
    ...
    ///   - type: Whether the purchase is a subscription or a one-time purchase.
    ///   - price: The price paid, in the currency identified by `currencyCode`.
    ///   - currencyCode: The ISO 4217 currency code of `price`.
    ///   - countryCode: The ISO 3166-1 country code of the storefront the purchase was made on. Default is `nil`.
    ...
    ///
    /// This function converts `price` to USD for revenue analytics. The conversion happens with hard-coded exchange rates that might be out of date.
    /// The SDK does not keep track of purchases you have already reported — repeatedly calling this method will result in multiple events.
    ...
    public static func purchaseCompleted(
        ...
        self.internalSignal(
            ...
            parameters: purchaseParameters(productID: productID, type: type, currencyCode: currencyCode, countryCode: countryCode)
    ...
    /// Sends a telemetry signal indicating that a user converted from a free trial to a paid purchase.
    ///
    /// - Parameters:
    ///   - productID: The identifier of the purchased product.
    ///   - type: Whether the purchase is a subscription or a one-time purchase.
    ...
    /// Sends a telemetry signal indicating that a free trial has started.
    ///
    /// - Parameters:
    ///   - productID: The identifier of the product the trial is for.
    ///   - type: Whether the trial is for a subscription or a one-time purchase.
    ///   - currencyCode: The ISO 4217 currency code the product is priced in.
    ///   - countryCode: The ISO 3166-1 country code of the storefront the trial was started on. Default is `nil`.
    ///   - parameters: Additional parameters to include with the signal. Default is an empty dictionary.
    ///   - customUserID: An optional custom user identifier. If provided, it overrides the default user identifier from the configuration. Default is `nil`.
    ///
    /// No `floatValue` is recorded because a free trial has no charge. Call ``convertedFromTrial(productID:type:price:currencyCode:countryCode:parameters:customUserID:)``
    /// when the trial converts to a paid purchase.
    @available(iOS 13, macOS 10.15, tvOS 13, watchOS 6, *)
    ...
    static func purchaseParameters(productID: String, type: PurchaseType, currencyCode: String, countryCode: String?) -> [String: String] {
        var parameters: [String: String] = [
            "TelemetryDeck.Purchase.type": type.rawValue,
            "TelemetryDeck.Purchase.productID": productID,
            "TelemetryDeck.Purchase.currencyCode": currencyCode,
        ]

        if let countryCode {
            parameters["TelemetryDeck.Purchase.countryCode"] = countryCode
        }
    ...
    static func priceInUSD(_ price: Decimal, currencyCode: String) -> Double {
        let priceValueInNativeCurrency = NSDecimalNumber(decimal: price).doubleValue

        if currencyCode == "USD" {
            return priceValueInNativeCurrency
        } else if let oneUSDExchangeRate = currencyCodeToOneUSDExchangeRate[currencyCode] {
            return priceValueInNativeCurrency / oneUSDExchangeRate
        } else {
            return 0
        }
    }
    ...
    extension TelemetryDeck {
        ...
        @available(
            *,
            deprecated,
            message:
                "Use 'purchaseCompleted(productID:type:price:currencyCode:countryCode:parameters:customUserID:)' instead. Note: the replacement methods do not detect free trials or trial conversions automatically – call 'freeTrialStarted(...)' when a trial begins and 'convertedFromTrial(...)' when it converts."
        )
        public static func purchaseCompleted(
    ...
    extension Transaction {
        ...
        func priceInUSD() -> Double {
            let priceValueInNativeCurrency = self.price ?? Decimal()

            if #available(iOS 16, macOS 13, tvOS 16, watchOS 9, *) {
                let currencyCode = self.currency?.identifier ?? ""
                return TelemetryDeck.priceInUSD(priceValueInNativeCurrency, currencyCode: currencyCode)
            } else {
