    ...
    extension TelemetryDeck {
        ...
        public static func purchaseCompleted(
            ...
            if #available(iOS 17.2, macOS 14.2, tvOS 17.2, visionOS 1.1, watchOS 10.2, *) {
                ...
                {
                    await reportFreeTrial(transaction: transaction, parameters: parameters, customUserID: customUserID)
                    return
                }
            } else {
                #if !os(visionOS)
            ...
            await reportPaidPurchase(transaction: transaction, parameters: parameters, customUserID: customUserID)
        }
        ...
        private static func reportFreeTrial(
            ...
            if let processor = await client.processor(ofType: TrialConversionProcessor.self) {
                await processor.freeTrialStarted(transaction: transaction)
            }
        }
    ...
    extension Transaction {
        ...
        func purchaseParameters() -> [String: String] {
            ...
            var params: [String: String] = [
                DefaultParams.Purchase.type.rawValue: self.subscriptionGroupID != nil ? "subscription" : "one-time-purchase",
                DefaultParams.Purchase.countryCode.rawValue: countryCode,
                DefaultParams.Purchase.productID.rawValue: self.productID,
            ]

            if #available(iOS 16, macOS 13, tvOS 16, watchOS 9, *) {
                if let currencyCode = self.currency?.identifier {
                    params[DefaultParams.Purchase.currencyCode.rawValue] = currencyCode
                }
            } else {
                #if !os(visionOS)
        ...
        func priceInUSD() -> Double {
            let nativePrice = NSDecimalNumber(decimal: self.price ?? Decimal()).doubleValue

            if #available(iOS 16, macOS 13, tvOS 16, watchOS 9, *) {
                if self.currency?.identifier == "USD" {
                    return nativePrice
                } else if let code = self.currency?.identifier,
                    let rate = Self.currencyCodeToOneUSDExchangeRate[code]
                {
                    ...
                    if self.currencyCode == "USD" {
                        return nativePrice
                    } else if let code = self.currencyCode,
                        let rate = Self.currencyCodeToOneUSDExchangeRate[code]
                    {
                        return nativePrice / rate
                    } else {
